/*
  ...
  - You are about to drop the `Occupational` table. If the table is not empty, all the data it contains will be lost.
  ...
  - You are about to drop the `UnitOccupational` table. If the table is not empty, all the data it contains will be lost.
  - You are about to drop the `UnitSector` table. If the table is not empty, all the data it contains will be lost.
...
-- DropForeignKey
ALTER TABLE `UnitOccupational` DROP FOREIGN KEY `UnitOccupational_occupationalId_fkey`;

-- DropForeignKey
ALTER TABLE `UnitOccupational` DROP FOREIGN KEY `UnitOccupational_unitCodeId_fkey`;

-- DropForeignKey
ALTER TABLE `UnitSector` DROP FOREIGN KEY `UnitSector_sectorId_fkey`;

-- DropForeignKey
ALTER TABLE `UnitSector` DROP FOREIGN KEY `UnitSector_unitCodeId_fkey`;
...
-- DropTable
DROP TABLE `UnitOccupational`;

-- DropTable
DROP TABLE `UnitSector`;
...
CREATE TABLE `AssetPermission` (
    ...
    UNIQUE INDEX `uix_permission_asset`(`permissionId`, `assetId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `AssetPermission` ADD CONSTRAINT `AssetPermission_permissionId_fkey` FOREIGN KEY (`permissionId`) REFERENCES `Permission`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
