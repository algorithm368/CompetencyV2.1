...
export function useCompetencyDetailError() {
   ...
   */
  const parseError = useCallback((error: ErrorInput, source?: CompetencySource, code?: string): ErrorInfo => {
   ...
   */
  const addError = useCallback((error: ErrorInput, source?: CompetencySource, code?: string): ErrorInfo => {
   ...
   */
  const validateCompetencyCode = useCallback((code: string, source: CompetencySource): ValidationResult => {
    // Basic validation
    ...
    const result = source === 'sfia' 
      ? validateSfiaCode(trimmedCode)
      : validateTpqiCode(trimmedCode);
   ...
   */
  const hasRecentErrorType = useCallback((type: ErrorInfo['type'], minutes: number = 5): boolean => {
