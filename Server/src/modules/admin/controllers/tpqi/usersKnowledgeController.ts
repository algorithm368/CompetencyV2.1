...
export class UsersKnowledgeController {
  ...
  static async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = req.body as Omit<import("@prisma/client_tpqi").users_knowledge, "id_users_knowledge">;
  ...
  static async update(req: Request, res: Response, next: NextFunction) {
    try {
      const actor = req.headers["x-actor-id"] as string;
      const id = Number(req.params.id);
      const updates = req.body as Partial<Omit<import("@prisma/client_tpqi").users_knowledge, "id_users_knowledge">>;
