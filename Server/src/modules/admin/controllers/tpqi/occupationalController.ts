...
import { OccupationalService } from "@Admin/services/tpqi/OccupationalService";

const service = new OccupationalService();

export class OccupationalController {
  ...
  static async getById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!item) {
        return res.status(404).json({ error: `Occupational with id ${id} not found` });
      }
      res.json(item);
    } catch (err) {
      next(err);
  ...
  static async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (err.code === "P2025") {
        return res.status(404).json({ error: `Occupational with id ${req.params.id} not found` });
      }
      next(err);
    }
  }
