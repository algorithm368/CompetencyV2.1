...
import { CareerService } from "@Admin/services/tpqi/CareerService";
...
export class CareerController {
  ...
  static async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const updates = req.body as Partial<Omit<import("@prisma/client_tpqi").Career, "id_career">>;
      const updated = await service.update(id, updates, actor);
      res.json(updated);
    } catch (err: any) {
      if (err.code === "P2025") {
        return res.status(404).json({ error: `Career with id ${req.params.id} not found` });
      }
      next(err);
    }
  }
