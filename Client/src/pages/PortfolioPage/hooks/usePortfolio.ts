...
export const usePortfolio = () => {
  ...
  const [completeData, setCompleteData] =
    useState<CompletePortfolioData | null>(null);
    ...
    async (userEmail: string) => {
      try {
        ...
        if (!validation.isValid) {
          throw new Error(
            validation.error || "Service configuration is invalid",
          );
        }

        // Fetch complete portfolio data
        const completePortfolio =
          await portfolioService.getCompletePortfolioData(userEmail);
        setCompleteData(completePortfolio);

        // Convert to UI format
        const uiPortfolio =
          portfolioService.convertToPortfolioData(completePortfolio);
  ...
  const hasData =
    portfolioData !== null &&
    (portfolioData.sfiaSkills.length > 0 ||
      portfolioData.tpqiCareers.length > 0);
