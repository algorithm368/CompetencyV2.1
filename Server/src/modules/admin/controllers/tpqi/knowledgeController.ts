...
import { KnowledgeService } from "@Admin/services/tpqi/KnowledgeService";
...
export class KnowledgeController {
  ...
  static async getById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!item) {
        return res.status(404).json({ error: `Knowledge with id ${id} not found` });
      }
      res.json(item);
    } catch (err) {
      next(err);
  ...
  static async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = req.body as Omit<import("@prisma/client_tpqi").Knowledge, "id_knowledge">;
  ...
  static async update(req: Request, res: Response, next: NextFunction) {
    try {
      const actor = req.headers["x-actor-id"] as string;
      const id = Number(req.params.id);
      const updates = req.body as Partial<Omit<import("@prisma/client_tpqi").Knowledge, "id_knowledge">>;
      const updated = await service.update(id, updates, actor);
      res.json(updated);
    } catch (err: any) {
      if (err.code === "P2025") {
        return res.status(404).json({ error: `Knowledge with id ${req.params.id} not found` });
      }
      next(err);
    }
  }
