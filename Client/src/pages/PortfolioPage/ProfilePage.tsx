...
const ProfilePage: React.FC = () => {
  const userId = 1;
  const foundUser: UserType | undefined = mockUsers.find(
    (u) => u.id === userId
  );
  const userOccupation: OccupationType | undefined = foundUser
    ? mockOccupations.find((occ) => occ.id === foundUser.occupationId)
    : undefined;
  ...
  const evidenceCount = Object.values(foundUser.evidenceUrls).filter(
    (url) => url.trim() !== ""
  ).length;
        ...
        {userOccupation && (
          <section className="bg-white p-8 rounded-2xl shadow-md">
            ...
            <div className="space-y-6">
              {Object.entries(levelProgressScreen).map(
                ([level, { total, completed }]) => {
                  const percent =
                ...
                <div className="grid grid-cols-1 lg:grid-cols-1 gap-8">
                  {userOccupation.skills.map((skill: SkillType) => {
                    const hasEvidence = Boolean(
                      foundUser.evidenceUrls[skill.id]?.trim() !== ""
                    );
                    return (
                      <div
                        key={skill.id}
                      ...
                      >
                        <div className="flex items-center justify-between mb-4">
                          <div>
                            ...
                            <p className="text-sm text-gray-500 mt-0.5">
                              Framework: {skill.framework} | Level:{" "}
                              {skill.level}
                            </p>
                          </div>
                          {hasEvidence ? (
                        ...
                        <details className="mb-4">
                          <summary className="cursor-pointer text-indigo-600 hover:underline mb-2">
                            Responsibilities
                          </summary>
