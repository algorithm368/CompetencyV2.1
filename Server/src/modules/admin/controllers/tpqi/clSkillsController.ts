...
export class ClSkillsController {
  ...
  static async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = req.body as Omit<import("@prisma/client_tpqi").CareerLevelSkill, "id_cl_skills">;
  ...
  static async update(req: Request, res: Response, next: NextFunction) {
    try {
      const actor = req.headers["x-actor-id"] as string;
      const id = Number(req.params.id);
      const updates = req.body as Partial<Omit<import("@prisma/client_tpqi").CareerLevelSkill, "id_cl_skills">>;
