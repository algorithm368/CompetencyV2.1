...
const SuccessState: React.FC<SuccessStateProps> = ({
  ...
  const resultAnalysis = useMemo(() => {
    const analysis = {
      total: items.length,
      frameworks: {
        sfia: items.filter(item => item.framework?.toLowerCase() === 'sfia').length,
        tpqi: items.filter(item => item.framework?.toLowerCase() === 'tpqi').length,
        other: items.filter(item => !['sfia', 'tpqi'].includes(item.framework?.toLowerCase())).length
      },
      qualityScore: 0,
      hasExactMatch: false,
  ...
  const handleViewDetails = useCallback((id: string) => {
    ...
    console.log(`Viewing details for: ${item?.name} (${item?.framework}) from query: ${query}`);
    onViewDetails(id);
  ...
  const resultInsights = useMemo(() => {
    ...
    if (resultAnalysis.frameworks.sfia > 0 && resultAnalysis.frameworks.tpqi > 0) {
      insights.push(`พบข้อมูลจากทั้ง SFIA (${resultAnalysis.frameworks.sfia}) และ TPQI (${resultAnalysis.frameworks.tpqi})`);
    } else if (resultAnalysis.frameworks.sfia > 0) {
      insights.push(`พบข้อมูลจาก SFIA เท่านั้น (${resultAnalysis.frameworks.sfia} รายการ)`);
    } else if (resultAnalysis.frameworks.tpqi > 0) {
      insights.push(`พบข้อมูลจาก TPQI เท่านั้น (${resultAnalysis.frameworks.tpqi} รายการ)`);
    }
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm text-gray-600">
              ...
              {/* Quality Score Badge */}
              <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getQualityBadgeStyle(resultAnalysis.searchRelevance)}`}>
