...
const detectErrorType = (error: string): ErrorType => {
  ...
  if (lowerError.includes('เครือข่าย') || lowerError.includes('เชื่อมต่อ') || 
      lowerError.includes('network') || lowerError.includes('connection')) return 'network';
      
  if (lowerError.includes('อย่างน้อย') || lowerError.includes('ตัวอักษร') || 
      lowerError.includes('ยาวเกินไป') || lowerError.includes('validation')) return 'validation';
  ...
  if (lowerError.includes('หมดเวลา') || lowerError.includes('ใช้เวลานานเกินไป') || 
      lowerError.includes('timeout')) return 'timeout';
      
  if (lowerError.includes('เซิร์ฟเวอร์') || lowerError.includes('ระบบ') || 
      lowerError.includes('server') || lowerError.includes('500')) return 'server';
      
  if (lowerError.includes('บ่อยเกินไป') || lowerError.includes('rate limit')) return 'rateLimit';
  
  if (lowerError.includes('ไม่ได้รับอนุญาต') || lowerError.includes('permission') || 
      lowerError.includes('unauthorized')) return 'permission';
        ...
        <div className="text-center space-y-3">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-center">
              <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${colorScheme.iconBg} ${colorScheme.text} border ${colorScheme.border}`}>
                ประเภท: {errorType}
              </span>
            </div>
          </div>
