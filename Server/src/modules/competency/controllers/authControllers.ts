...
export const loginWithGoogle = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.status(StatusCodes.OK).json({ user, accessToken, expiresIn: process.env.EXPIRES_IN || "3600" });
  } catch (err: any) {
...
export const refreshAccessToken = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.status(StatusCodes.OK).json({ accessToken: newAccessToken, expiresIn: process.env.EXPIRES_IN || "3600" });
  } catch (err: any) {
    console.error("Error during refresh token:", err);
    res.status(StatusCodes.UNAUTHORIZED).json({ message: err.message || "Refresh token failed" });
  }
};
