...
import { UnitCode, CreateUnitCodeDto, UpdateUnitCodeDto, UnitCodePageResult } from "@Types/tpqi/unitCodeTypes";
...
export function useUnitCodeManager(
    ...
    const fetchPage = async (pageIndex: number, pageSize: number): Promise<UnitCodePageResult> => {
    ...
    const deleteUnitCode = useMutation<void, Error, number>({
        mutationFn: (id) => UnitCodeService.delete(id),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["unitCodes"] });
