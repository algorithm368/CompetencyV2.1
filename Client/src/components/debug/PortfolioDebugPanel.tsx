...
const PortfolioDebugPanel: React.FC = () => {
  ...
  const baseApiUrl = process.env.REACT_APP_API_BASE_URL || "http://localhost:3000";
  ...
  const testSfiaEndpoint = async () => {
    try {
      ...
      const service = new GetSfiaSummaryService(baseApiUrl, accessToken);
  ...
  const testTpqiEndpoint = async () => {
    try {
      ...
      const service = new GetTpqiSummaryService(baseApiUrl, accessToken);
      const response = await service.getUserSummary();
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {!accessToken && (
          <div className="bg-red-50 border border-red-200 rounded p-3 mb-4">
            <p className="text-red-700">No access token available. Please log in first.</p>
          </div>
        )}
            ...
            {sfiaData && (
              <div className="bg-blue-50 border border-blue-200 rounded p-3">
                ...
                <div className="text-sm text-blue-700 space-y-1">
                  <p><strong>Success:</strong> {String(sfiaData.success)}</p>
                  <p><strong>Message:</strong> {sfiaData.message || "None"}</p>
                  <p><strong>Data Structure:</strong></p>
                  <pre className="text-xs bg-white p-2 rounded border overflow-auto max-h-40">
                    {JSON.stringify(sfiaData, null, 2)}
                  </pre>
                </div>
              </div>
            )}
            ...
            {tpqiData && (
              <div className="bg-green-50 border border-green-200 rounded p-3">
                ...
                <div className="text-sm text-green-700 space-y-1">
                  <p><strong>Success:</strong> {String(tpqiData.success)}</p>
                  <p><strong>Message:</strong> {tpqiData.message || "None"}</p>
                  <p><strong>Data Structure:</strong></p>
                  <pre className="text-xs bg-white p-2 rounded border overflow-auto max-h-40">
                    {JSON.stringify(tpqiData, null, 2)}
                  </pre>
                </div>
              </div>
            )}
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded">
          ...
          <div className="text-sm text-gray-600 space-y-1">
            ...
            <p><strong>SFIA Endpoint:</strong> {baseApiUrl}/api/sfia/summary/user</p>
            <p><strong>TPQI Endpoint:</strong> {baseApiUrl}/api/tpqi/summary/user</p>
