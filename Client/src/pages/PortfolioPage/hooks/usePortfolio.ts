 ...
 */
export const usePortfolio = (baseApiUrl: string, accessToken: string | null) => {
  ...
  const portfolioService = useMemo(() => {
    return new PortfolioService(baseApiUrl, accessToken);
  }, [baseApiUrl, accessToken]);

  /**
   * Fetches portfolio data from the API.
   * @param userEmail - The user's email address
   */
  const fetchPortfolioData = useCallback(async (userEmail: string) => {
    try {
      setLoading(true);
      setError(null);
   ...
   * @param userEmail - The user's email address
   */
  const refreshPortfolio = useCallback((userEmail: string) => {
  ...
  const hasData = portfolioData !== null && 
    (portfolioData.sfiaSkills.length > 0 || portfolioData.tpqiCareers.length > 0);

  /**
   * Gets recommendations based on current portfolio data.
   */
  const recommendations = completeData ? 
    portfolioService.generateRecommendations(completeData) : [];

  /**
   * Checks if data is stale (older than 5 minutes).
   */
  const isDataStale = lastFetched ? 
    (new Date().getTime() - lastFetched.getTime()) > 5 * 60 * 1000 : true;
